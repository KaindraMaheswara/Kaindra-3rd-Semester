...
int main(){
    ListDin kelas1, kelas2, output;
    int i, j, n;
    readList(&kelas1);
    readList(&kelas2);
    CAPACITY(output) = CAPACITY(kelas1);
    n = 0;
    printList(kelas1);
    printList(kelas2);
    for (i = 0; i <= getLastIdx(kelas1); i++){
        for (j = 0; j <= getLastIdx(kelas2); j++){
            if (ELMT(kelas1, i) == ELMT(kelas2, j)){
                insertLast(&output, ELMT(kelas1, i));
                n += 1;
            }
        }
    }
